...
#    fp = [0.0, 0.08, 0.024, 0.24, 0.8, 2.4]
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            path2 = args.savefile + '/plots/HISTOGRAM'
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if os.path.exists(f):
                ...
                y = np.asarray(order_data)
                weights = np.ones_like(y)/len(y)/0.02
                y, x = np.histogram(y, range=[0.,1.], bins=50, weights=weights) 
                ...
                # Curve fitting
                print f
                save_data(f, x[:-1], y, 'HISTOGRAM')
            
            print np.sum(y*np.diff(x))
            # Plot the data to the subplot
            c = next(color)
            
            ax[subcnt].plot(x[:-1], y, \
        ...
        ax[subcnt].set_xlabel('$\\rho \\sigma^2$', weight='bold', size=30)
        ax[subcnt].set_ylabel('$P(\\rho \\sigma^2)$', weight='bold', size=30)
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Density Distribution, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
