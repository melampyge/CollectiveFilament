    ...
    for d in density:
        ...
        for f in fp:
            ...
            for k in kappa:
                ...
                path = generate_path(str(d),str(k),str(f),"CLUSTER")
                data = np.transpose(np.loadtxt(path, dtype=float))
                x = data[0]
                y = data[1]
                
                ## Curve fitting
                print Pe, persistence
                if Pe < 1.:
                    popt, pcov = curve_fit(stretch_exp_law, x, y)
                    yfit = stretch_exp_law(x, popt[0], popt[1], popt[2])
                    print popt[1]
#                    if persistence < 0.3:
...
#                        yfit = explaw(x, popt[0], popt[1]) 
                    ...
                    if persistence < 0.3:
                            ...
                            yfit = stretch_exp_law(x, popt[0], popt[1], popt[2])   
                            print popt[1]
#                        popt, pcov = curve_fit(mixed_power_law_with_exp_tail, x, y)
#                        yfit = mixed_power_law_with_exp_tail(x, popt[0], popt[1], popt[2], popt[3], popt[4])
                    else:
                        if Pe > 4300.:
                            popt, pcov = curve_fit(stretch_exp_law, x, y)
                            yfit = stretch_exp_law(x, popt[0], popt[1], popt[2])
...
#                            yfit = power_law(x, popt[0], popt[1])
        ...
        for i, f in enumerate(fp):
            ...
            ax[i].set_xlabel('$m$', weight='bold', size=40)
        ...
        ax[0].set_ylabel('$P(m)$', weight='bold', size=40)
        
        plt.setp(ax[1].get_yticklabels(),visible=False)
        #plt.setp(ax[2].get_yticklabels(),visible=False)

        ax[1].legend(bbox_to_anchor=(0.58,0.,0.45,1.), loc=2, borderaxespad=0., prop={'size': 22}, mode="expand", frameon=False)
        
#        plt.figtext(0.5, 1.1, \
#            'Cluster Mass Distribution, ' + labels['density'] + ' = ' + "{0:.1f}".format(d), size=35, weight='bold')        
        ## Save the previous figure to this file
        path1 = save_base + '/plots'
        path2 = save_base + '/plots/CLUSTER'
