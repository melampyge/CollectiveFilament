...
def plot_data(folderbase, savebase):
    ...
    popt, pcov = curve_fit(stretch_exp_law, m[:40], pmg[:40])
    pmg_fit = stretch_exp_law(m[:40], popt[0], popt[1], popt[2])
    ...
    popt, pcov = curve_fit(power_law, m[:200], pmc[:200])
    pmc_fit = power_law(m[:200], popt[0], popt[1])
    print 'Power law fit for the cluster phase with ', popt[0], popt[1]
    ...
    popt, pcov = curve_fit(power_law, m[:170], pmgc[:170])
    pmgc_fit = power_law(m[:170], popt[0], popt[1])
    print 'Power law fit for the giant cluster phase with ', popt[0], popt[1]
    ...
    ax0.loglog(m[:40], pmg_fit, label="_nolegend_", linewidth=2.0, color='blue')
    ax0.loglog(m, pmg, 'o', label="_nolegend_", color='blue', markersize=7)
    
    ax0.loglog(m[:200], pmc_fit, label="_nolegend_", linewidth=2.0, color='green')
    ax0.loglog(m, pmc, 'o', label="_nolegend_", color='green', markersize=7)

    ax0.loglog(m[:170], pmgc_fit, label="_nolegend_", linewidth=2.0, color='red')    
