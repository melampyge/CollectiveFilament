    ...
    for d in density:
        ...
        for f in fp:
            ...
            for k in kappa:
                ...
                file_path = data_path + '/CLUSTER/avg_size_extended.txt'
                if os.path.exists(file_path) == False:    
                    file_path = data_path + '/CLUSTER/avg_size.txt.gz'
                else:
...
#                    data += 0.6
        ...
        for i, f in enumerate(fp):
            ...
            for j, k in enumerate(kappa):
                ...
                ## Plot the data
                if labelon:      
                    ax[0].errorbar(x,yf[j],yerr=yf_std[j],fmt='--o', \
...
#            'Rotational Force, ' + labels['density'] + ' = ' + "{0:.2f}".format(d), size=35, weight='bold') 
        ...
        if os.path.exists(path2) == False: 
            os.mkdir(path2)                
        plt.savefig(path2 + '/paper_SIZE_set_' + \
