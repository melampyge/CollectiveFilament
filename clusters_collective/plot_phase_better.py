    ...
    for d in density:
        ...
        for f in fp:
            ...
            for k in kappa:
                ...
                elif os.path.exists(file_path) == False and os.path.exists(file_path+'.gz') == True:
                    os.system('gzip -d ' + file_path + '.gz')
                    data = np.loadtxt(file_path, dtype=float)     
                else:
                    data = 0.
                print data
        ...
        ## Plot properties
        pe = convert_to_dimensionless(f, 'fp')
        ax[0].set_title(r'Average lifetime of clusters', weight='bold', size=20)
        ax[0].set_xlabel(r'Pe', weight='bold', size=20)
        ...
        ax[0].set_ylabel(r'$Lifetiem$', weight='bold', size=20)

        ax[0].legend(bbox_to_anchor=(1.02,0.,0.45,1.), loc=2, borderaxespad=0., prop={'size': 20}, mode="expand", frameon=False)
        
        plt.figtext(0.05, 1.1, \
            'Size, ' + labels['density'] + ' = ' + "{0:.2f}".format(d), size=35, weight='bold') 
        ...
        if os.path.exists(path2) == False: 
            os.mkdir(path2)                
        plt.savefig(path2 + '/LIFETIME_better_set_' + \
