...
def compare_clusters(cluster_idx, old_cluster_idx, nfil, lx, ly, props_group, unique_id):
...
#            print 'Maximum number of overlapping filaments is: ', maxi_intersect
    ...
    idx_h5 = np.zeros(len(cluster_idx), dtype=int)
    size_h5 = np.zeros(len(cluster_idx), dtype=int)
    comx_h5 = np.zeros(len(cluster_idx), dtype=float)
    comy_h5 = np.zeros(len(cluster_idx), dtype=float)
    rgysq_h5 = np.zeros(len(cluster_idx), dtype=float)
    planarity_h5 = np.zeros(len(cluster_idx), dtype=float)
    straightness_h5 = np.zeros(len(cluster_idx), dtype=float)
    swirliness_h5 = np.zeros(len(cluster_idx), dtype=float)
    enstrophy_h5 = np.zeros(len(cluster_idx), dtype=float)
    fils = []
   
    for j in range(len(cluster_idx)):
        idx_h5[j] = cluster_idx[j][0]
        size_h5[j] = cluster_idx[j][1]
        comx_h5[j] = cluster_idx[j][2]
        comy_h5[j] = cluster_idx[j][3]
        rgysq_h5[j] = cluster_idx[j][4]
        planarity_h5[j] = cluster_idx[j][5]
        straightness_h5[j] = cluster_idx[j][6]
        swirliness_h5[j] = cluster_idx[j][7]
        enstrophy_h5[j] = cluster_idx[j][8]
        fil_hold = []
        for kj in range(len(cluster_idx[j][9])):
            fil_hold.append(cluster_idx[j][9][kj]) 
        fils.append(fil_hold)
             
    props_group.create_dataset('idx', data=idx_h5, compression='gzip')
    props_group.create_dataset('size', data=size_h5, compression='gzip')
    props_group.create_dataset('comx', data=comx_h5, compression='gzip')
    props_group.create_dataset('comy', data=comy_h5, compression='gzip')
    props_group.create_dataset('rgysq', data=rgysq_h5, compression='gzip')
    props_group.create_dataset('planarity', data=planarity_h5, compression='gzip')
    props_group.create_dataset('straightness', data=straightness_h5, compression='gzip')
    props_group.create_dataset('swirliness', data=swirliness_h5, compression='gzip')
    props_group.create_dataset('enstrophy', data=enstrophy_h5, compression='gzip')

    fil_grp = props_group.create_group('filament_list')
...
def cluster_analysis(charfile, headerfile, ofname, nfil, nskip, dcrit, lcrit, ccrit, wcrit):
    ...
    for i in range(nsteps):
        ...
        ofile = h5py.File(ofname + '/cluster_' + str(tstep) + '.hdf5', 'w')
        beads_grp = ofile.create_group('beads')
        x_h5 = np.zeros(len(x),dtype=float)
        y_h5 = np.zeros(len(x),dtype=float)
        phi_h5 = np.zeros(len(x),dtype=float)
        clusters_atm_h5 = np.zeros(len(x),dtype=float)
        for ii in range(len(x)):
            x_h5[ii] = x[ii]
            y_h5[ii] = y[ii]
            phi_h5[ii] = phi[ii]
            clusters_atm_h5[ii] = clusters_atm[ii]
        beads_grp.create_dataset('x', data=x_h5, compression='gzip')
        beads_grp.create_dataset('y', data=y_h5, compression='gzip')
        beads_grp.create_dataset('phi', data=phi_h5, compression='gzip')
        beads_grp.create_dataset('idx', data=clusters_atm_h5, compression='gzip')
        ...
        size_grp = ofile.create_group('size')
        size_grp.create_dataset('size', data=cl_mol, compression='gzip')
        t[i] = tstep
        ncli = len(cl_mol)
        ncl[i] = ncli
        clmax[i] = max(cl_mol)
        ...
        xcluster, ycluster, isolated = correct_cluster_pbc(x,y,lx,ly,clusters,nfil,dcrit)
        ...
        props_grp = ofile.create_group('props')
...
#            trans_cluster_idx = np.transpose(cluster_idx)
#            idx_h5 = np.asarray(trans_cluster_idx[0], dtype=int)
#            size_h5 = np.asarray(trans_cluster_idx[1], dtype=int)
#            comx_h5 = np.asarray(trans_cluster_idx[2], dtype=float)
#            comy_h5 = np.asarray(trans_cluster_idx[3], dtype=float)
#            rgysq_h5 = np.asarray(trans_cluster_idx[4], dtype=float)
#            planarity_h5 = np.asarray(trans_cluster_idx[5], dtype=float)
#            straightness_h5 = np.asarray(trans_cluster_idx[6], dtype=float)
#            swirliness_h5 = np.asarray(trans_cluster_idx[7], dtype=float)
#            enstrophy_h5 = np.asarray(trans_cluster_idx[8], dtype=float)
        ...
        unique_id = compare_clusters(cluster_idx, old_cluster_idx, nfil, lx, ly, props_grp, unique_id)
        ofile.close()    
        old_cluster_idx = cluster_idx
