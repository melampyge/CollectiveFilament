...
def main(): 
    ...
    total_subplots_in_x = 5           # Total number of subplots in the x direction
    ...
    for i, grp in group:
        ...
        for j, f in enumerate(grp['folder']):
            ...
            #clsizes = np.zeros((M+1,1))
            
            y = []
            if os.path.exists(f):
                
                order_data = pd.read_csv(f, sep='\t', header=None) 
                x = np.asarray(order_data[0], dtype=float)
                for siz_idx, siz_cnt in enumerate(order_data[1]):
                    y.append( float(siz_cnt[1:-1])*float(x[siz_idx])/float(M) )
                
                save_data(f, x, y, 'CLUSTER')
                
                #y_binned = np.histogram(y, bins=50, normed=True)
...
#                            yfit = powerwithexp(x, popt[0], popt[1])
    ...
    #                            yfit = powerwithupexp(x, popt[0], popt[1], popt[2], popt[3], popt[4])
            ...
            ax[subcnt].loglog(x, y, \
            label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]), linewidth=2.0, color=c)
            
            ax[subcnt].loglog(x, yfit, '--', \
            label='_nolegend_', linewidth=2.0, color=c)
        ...
        ax[subcnt].set_xlabel('m', weight='bold', size=30)
        ax[subcnt].set_ylabel('<P(m)>', weight='bold', size=30)
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Cluster Mass Distribution, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
