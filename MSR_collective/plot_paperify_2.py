...
#        i0prev = i
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if C_cnt == np.size(fp):
                ...
                c = next(color)
                #print x, '\t', C, '\n'
                label_fac = grp[constants[1]].iloc[j]
                if int(label_fac) == 7:
                    label_fac = 8.00
                elif int(label_fac) == 4:
                    label_fac = 5.00
                ax[subcnt].loglog(x, C, 'o', \
                    label=labels[constants[1]] + " = " + "{0:.1f}".format(label_fac), color=c, markersize=15)
#                ax[subcnt].plot(x, C,  \
#                linewidth = 1., color=c)
                C = []
                x = []
        ...
        #ax[subcnt].set_title( labels[compareby] + " = " + "{0:.2f}".format(i), weight='bold', size='medium' )
        ax[subcnt].legend(bbox_to_anchor=(0.02,0.,0.45,1.), loc=2, borderaxespad=0., prop={'size': 18}, mode="expand", frameon=False)
        ax[subcnt].set_xlabel(r'$Pe$', weight='bold', size=40)
        ax[subcnt].set_ylabel(r'$D_{r} \tau$', weight='bold', size=40)
        ...
        ax[subcnt].yaxis.set_ticks( [10**-2, 10**0, 10**2] )
        ax[subcnt].tick_params(axis='both', which='major', labelsize=30)
        ...
        path2 = args.savefile + '/plots/phase/ROT_DIFF'
        if os.path.exists(path1) == False:
            os.mkdir(path1)
        if os.path.exists(path2) == False:  
            os.mkdir(path2)
            
        plt.savefig(path2 + '/paper_from_msr_set_' + \
            labels[compareby] + '.png', dpi=200, bbox_inches='tight', pad_inches=0.08)
        plt.savefig(path2 + '/paper_from_msr_set_' + \
            labels[compareby] + '.eps', dpi=200, bbox_inches='tight', pad_inches=0.08)            
        plt.clf()
