...
def main(): 
    ...
    for i, grp in group:
        ...
        for j, f in enumerate(grp['folder']):
            ...
            ofile = open(f+'/rot_dif_from_msr.data','w')
            ...
            if os.path.exists(f):
                ...
                save_data(f, x, y, z, 'MSR')
 
            # Plot the data to the subplot
            c = next(color)
            print np.shape(x), np.shape(y), np.shape(z)
            ax[subcnt].loglog(x,z, label="_nolegend_", color=c)
            ax[subcnt].loglog(x,y, 'o', label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]), linewidth=2., color=c)
        ...
        ax[subcnt].set_ylabel('$\\Delta \\theta^2/L^2$', weight='bold', size='large')
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Mean Square Rotation, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
