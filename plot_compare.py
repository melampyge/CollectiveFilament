...
def func(x, a):
    ...
    return 2 * x**(-2) * special.gammainc(2/a, x**a) / a
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            plt.savefig(path2 + '/nematic_order_set_' + \
                labels[constants[0]] + '_' + "{0:.1f}".format(i0prev) + '.png', dpi=200, bbox_inches='tight', pad_inches=0.08)
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if os.path.exists(f):
                ...
                popt, pcov = curve_fit(func, xdata[2:-4,0], ydata[2:-4,0])
        ...
        if subcnt == i0tot:
            ...
            'Nematic Order Parameter, ' + labels[constants[0]] + ' = ' + "{0:.1f}".format(i0prev), size='xx-large', weight='bold')
