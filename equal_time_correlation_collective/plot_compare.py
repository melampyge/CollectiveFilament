...
def main(): 
    ...
    jfile = open('/usr/users/iff_th2/duman/RolfData/many_polymers_5/plots/corr_len_from_polarity_corr.data', 'w')
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            path2 = args.savefile + '/plots/POLARITY_CORRELATION'
        ...
        for j, f in enumerate(grp['folder']):
            ...
            # Write the correlation length for the given folder
            ofile = open(f+'/polar_corr_len.data','w')
            ...
            if os.path.exists(f):
                ...
                y = order_data['pol_corrlation'] 
                
                 # Curve fitting
                popt, pcov = curve_fit(explaw, x, y)
                yfit = explaw(x, popt[0], popt[1])
                ofile.write(str(popt[1]))
                ofile.write('\n')
                jfile.write(str(grp['xi_L'].iloc[j]) + '\t' + str(i[1]) + '\t' + str(popt[1]) + '\n')
                
                save_data(f, x, y, 'POLARITY_CORRELATION')
                
            #print subcnt
            
            # Plot the data to the subplot
            c = next(color)
            ax[subcnt].semilogx(x,y, label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]), linewidth=2., color=c)
            ax[subcnt].semilogx(x,yfit, '--', label='_nolegend_', linewidth=2., color=c)
        ...
        ax[subcnt].set_xlabel('$r/\\sigma$', weight='bold', size=30)
        ax[subcnt].set_ylabel('$g_{p}(r)$', weight='bold', size=30)
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Polarity Correlation, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
