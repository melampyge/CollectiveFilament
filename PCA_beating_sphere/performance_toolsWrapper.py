import ctypes
lib = ctypes.cdll.LoadLibrary('/usr/users/iff_th2/isele/Applications/Scripts/PCA_beating_sphere/libperformance_tools.so')
...
class Performance_tools(object):
    ...
    def blocking_method(self, blockdata, block_std, block_uncert, n, av, std):

        # transform arrays to ctypes
        blockdatac = blockdata.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        block_stdc = block_std.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        block_uncertc = block_uncert.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        # transform integers and doubles to ctypes
        nc = ctypes.c_int(n)
        avc = ctypes.c_double(av)
        stdc = ctypes.c_double(std)
        lib.blocking_method(self.obj, blockdatac, block_stdc, block_uncertc, nc, ctypes.byref(avc), ctypes.byref(stdc))

        return avc.value, stdc.value
