...
def save_data(f1, f2, x, y1, ystd1, y2, ystd2, out_fname):
    ...
    for jj in range(len(x)):
        save_file.write(str(x[jj]) + '\t\t' + str(y1[jj]) \
            + '\t\t' + str(ystd1[jj]) + '\t\t' + str(y2[jj]) \
            + '\t\t' + str(ystd2[jj]) + '\n')
    save_file.close()    
...
def main(): 
    ...
    parser.add_argument("analysis_1", help="Type of analysis")
    parser.add_argument("analysis_2", help="Type of analysis")
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            ax[subcnt].legend(bbox_to_anchor=(1.05,0.,0.5,1.), loc=2, borderaxespad=0., prop={'size': 20}, mode="expand")
            ...
            plt.savefig(path2 + '/order_both_set_' + \
        ...
        for j, f in enumerate(grp['folder']):
            ...
            f1 = f + args.analysis_1+'.data'
            f2 = f + args.analysis_2+'.data'
            
            if os.path.exists(f1):
                order_data = pd.read_csv(f1, sep='\t', skiprows=4, header=None) 
                cols = np.arange(1,len(order_data.columns))
                t = order_data[0]
                y1 = order_data[cols].mean(axis=0)
                ystd1 = y1/np.sqrt(len(t))
                ystd1.index = cols-1
                order_data = pd.read_csv(f1, sep='\t', skiprows=3, nrows=1, header=None)
                x = np.asarray(order_data[cols].transpose())
                #lnx = np.log(x)
                
                order_data = pd.read_csv(f2, sep='\t', skiprows=4, header=None) 
                cols = np.arange(1,len(order_data.columns))
                t = order_data[0]
                y2 = order_data[cols].mean(axis=0)
                ystd2 = y2/np.sqrt(len(t))
                ystd2.index = cols-1
                y1.index = cols-1
                y2.index = cols-1
                
#                xs = np.zeros((len(x)),dtype=float)
#                for jjj in range(len(x)):
#                    xs[jjj] = x[jjj]
                save_data(f1, f2, x.flatten(), y1, ystd1, y2, ystd2, 'ORDER_PARAMETER')

            # Plot the data to the subplot
            c = next(color)
            
            ax[subcnt].errorbar(x, y1, yerr = ystd1, \
                label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_p", linewidth=2., color=c, marker ='o')
            ax[subcnt].errorbar(x, y2, yerr = ystd2, \
                label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_n", linewidth=2., color=c, marker ='*')
        ...
        ax[subcnt].set_xlabel('$w/\\sigma$', weight='bold', size=30)
        ax[subcnt].set_ylabel('$S_{q}$', weight='bold', size=30)
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Order Parameter, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
