...
#    fp = [0.0, 0.08, 0.024, 0.24, 0.8, 2.4]
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            plt.savefig(path2 + '/nematic_order_set_' + \
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if os.path.exists(f+'/ORDER_PARAMETER'):
                ofile_c = open(f+'/ORDER_PARAMETER/nematic_corr_len.data','w')
                ofile_e = open(f+'/ORDER_PARAMETER/nematic_exp.data','w')
...
#            label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_f", linewidth=1., color=c)
        ...
        ax[subcnt].set_ylabel('$S_{2}$', weight='bold', size='large')
