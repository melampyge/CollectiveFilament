...
def save_data(f, x, y, ystd, yf, out_fname):
    ...
    save_path = out_fname + '/polar_' + d + '_' + k + '_' + fp + '.data'
    save_file = open(save_path, 'w')
    for jj in range(len(y)):
        save_file.write(str(x[jj]) + '\t\t' + str(y[jj]) + '\t\t' + str(ystd[jj]) + '\t\t' + str(yf[jj]) + '\n')
    save_file.close()    
...
def main(): 
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            plt.savefig(path2 + '/polar_order_set_' + \
        ...
        for j, f in enumerate(grp['folder']):
            #pr( j , i
            #print f
            if os.path.exists(f+'/ORDER_PARAMETER'):
                ofile_c = open(f+'/ORDER_PARAMETER/polar_corr_len.data','w')
                ofile_e = open(f+'/ORDER_PARAMETER/polar_exp.data','w')
            ...
            if os.path.exists(f):
                ...
                popt, pcov = curve_fit(stretch_exp_law, xdata, ydata)
                yfit = stretch_exp_law(xdata, popt[0], popt[1], popt[2])
                ofile_c.write(str(popt[2]))
                ofile_c.write('\n')
                ofile_e.write(str(popt[1]))
                ofile_e.write('\n')
                print f, popt[1]
                
#                print np.shape(x.flatten()), np.shape(y), np.shape(ystd), np.shape(yfit)
#                print x.flatten()
#                print y
#                print ystd
#                print yfit
                save_data(f, x.flatten(), y, ystd, yfit, 'ORDER_PARAMETER')
...
#                    yfit = stretchexp(xdata, popt[0], popt[1], popt[2])
            ...
            label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_f", linewidth=2., color=c)
...
#            label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_f", linewidth=1., color=c)
        ...
        ax[subcnt].set_ylabel('$S_{1}$', weight='bold', size='large')
            
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+2*ax_sep, \
            'Polar Order Parameter, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size='xx-large', weight='bold')
