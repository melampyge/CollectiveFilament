...
#                print popt[1]
                ...
                yk1.append(y1)
                yk1_std.append(ystd1)
                yk2.append(y2)
                yk2_std.append(ystd2)
            
            ## Save as a function of propulsion force
            yf1.append(yk1)
            yf1_std.append(yk1_std)
            yf2.append(yk2)
            yf2_std.append(yk2_std)
        
        print np.shape(yf1), np.shape(yf2), np.shape(yf1_std), np.shape(yf2_std)
        ...
        for i, f in enumerate(fp):
            ...
            for j, k in enumerate(kappa):
                ...
                ## Plot the data
                c = next(color)
                ax[i].errorbar(x,yf1[i][j],yerr=yf1_std[i][j],marker='o', \
                    label=labels['xi_L']+'{0:.1f}'.format(xil),linewidth=2.,color=c)
                    
                ax[i].errorbar(x,yf2[i][j],yerr=yf2_std[i][j],marker='*', \
                    label='_nolegend_',linewidth=2.,color=c,linestyle='--') 
            ...
            ax[i].set_xlabel('$w/\\sigma$', weight='bold', size=30)
            ax[i].set_xlim((0,1))
        ...
        ax[0].set_ylabel('$S_{q}$', weight='bold', size=30)
        ...
        plt.figtext(0.6, 1.1, \
            'Order Parameter, ' + labels['density'] + ' = ' + "{0:.2f}".format(d), size=35, weight='bold')        
        ## Save the previous figure to this file
        path1 = save_base + '/plots'
        path2 = save_base + '/plots/ORDER_PARAMETER'
