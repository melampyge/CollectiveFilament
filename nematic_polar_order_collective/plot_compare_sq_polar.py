...
#    fp = [0.0, 0.08, 0.024, 0.24, 0.8, 2.4]
    ...
    jfile = open('/usr/users/iff_th2/duman/RolfData/many_polymers_5/plots/corr_len_from_sq_polar.data', 'w')
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            plt.savefig(path2 + '/polar_order_set_2_' + \
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if os.path.exists(f):
                ...
                popt, pcov = curve_fit(explaw, xdata, ydata)
                yfit = explaw(xdata, popt[0], popt[1])
...
#            label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]) + "_f", linewidth=1., color=c)
        ...
        ax[subcnt].set_ylabel('$S_{1}^2$', weight='bold', size='large')
