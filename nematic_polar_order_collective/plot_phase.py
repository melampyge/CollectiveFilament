...
def main(): 
    ...
    # Determine the minimum and maximum values for the order parameter
    ...
    for i, grp in group:
        ...
        for j, f in enumerate(grp['folder']):
            ...
            if os.path.exists(f):
                #order_data = pd.read_csv(f, sep='\t', skiprows=4, header=None) 
                order_data = np.loadtxt(f, skiprows=4, usecols=(9,))
                #cols = np.arange(1,len(order_data.columns))
                y = order_data.mean(axis=0)
                S_av[j] = y
            else:
            ...
            vmin=minzprev, vmax=maxzprev, s=sfac, edgecolors='None', cmap=plt.cm.brg)
        ...
        ax[subcnt].set_xlim((minx-1,maxx+1))
        ax[subcnt].set_ylim((miny-1,maxy+1))
        ax[subcnt].xaxis.set_ticks( np.arange(int(minx),int(maxx+1),int((maxx-minx)/4)) )
        ax[subcnt].yaxis.set_ticks( np.arange(int(miny),int(maxy+2),int((maxy-miny)/3)) )
        
    cax_1 = plt.axes([subp.xbeg+ax_len+0.04, subp.ybeg-ax_len-ax_sep, 0.01, 2*ax_len+ax_sep])
    plt.colorbar(line, cax=cax_1)
    cax_1.tick_params(width=0.4, labelsize=8)
    cax_1.set_title('$S_{1}$',fontsize=12)
    
    plt.figtext(subp.xbeg-ax_len, subp.ybeg+ax_len+0.15, \
        'Polar Order Parameter', size='x-large', weight='bold')
    plt.figtext(subp.xbeg-ax_len+0.08, subp.ybeg+ax_len+0.1, \
        'over bins of 3.5L', size='large')
    ...
    plt.savefig(path2 + '/polar_order_set_' + \
        labels[compareby] + '_L_3.5.png', dpi=200, bbox_inches='tight', pad_inches=0.08)
