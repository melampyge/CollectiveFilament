...
def get_order_parameter(x,y,phi,lx,ly):
    ...
    # prepare file for cpp code
    rc = 4.499*lrod
    cos = np.cos(2*phi)
    sin = np.sin(2*phi)
    nall = len(x)
    # write to file
    ofile = open('tmp.data', 'w')
    ofile.write(str(nall) + ' ' + str(rc) + ' ' + str(lx) + ' ' + str(ly) + '\n')
    for i in range(nall):
        ofile.write(str(x[i]) + ' ' + str(y[i]) + ' ' + str(cos[i]) + ' ' + str(sin[i]) + '\n')
    ofile.close()
    # start cpp code
    os.system('~/Applications/Scripts/compute_order_parameter/com_order')
    # read in the results
    ifile = open('tmp2.data')
    line = ifile.readline()
    line = line.split()
    Si = float(line[0])
    # the following commented section is handled by a cpp code
    """
    # generate a linked list
    rc = 4.499*lrod
