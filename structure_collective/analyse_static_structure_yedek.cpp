...
void compute_static_structure (double *S, double **x, double **y, int natoms, int Nbins, double bin_width, double delk) {
  /* calculate static structure per timestep with a running average */

  for (int nx = 0; nx < Nbins; nx++) {
    double kx = delk*nx;

      for (int ny = 0; ny < Nbins; ny++) {
	double ky = delk*ny;
...
int main (int argc, char *argv[]) {
  ...
  */
  ...
  for (int step = 0; step < nsteps; step++) {
    ...
    xt[0] = new double[natoms];
    for (int i = 0; i < natoms; i++) xt[0][i] = 0.; 

    double **yt = new double*[1];
    yt[0] = new double[natoms];
    for (int i = 0; i < natoms; i++) yt[0][i] = 0.; 
    ...
    compute_static_structure(S, xt, yt, natoms, Nbins, bin_width, delk);
