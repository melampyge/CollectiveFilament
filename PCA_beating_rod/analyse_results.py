...
try:
    ...
    print 'Usage: ' + sys.argv[0] + '        amplitude evolution file           eigenvalue file        correlaton file'
    exit()
...
def main():
    ...
    x1, y1 = reconstruct_principal_modes(eigvec[0], 2*eigval[0])
    x2, y2 = reconstruct_principal_modes(eigvec[1], 2*eigval[1])
    x3, y3 = reconstruct_principal_modes(eigvec[2], 2*eigval[2])
    x4, y4 = reconstruct_principal_modes(eigvec[3], 2*eigval[3])
    ...
    ### write down correlation functions to file
    ofile = open('correlations.data', 'w')
    ofile.write('Auto and cross correlations of the first two amplitudes\n\n')
    ofile.write('t\tac1\tstd\tac2\tstd\tcc12\tstd\n')
    for i in range(len(tc)):
        ofile.write(str(tc[i]) + '\t' + str(ac1[i]) + '\t' + str(ac1_std[i]) + '\t' + str(ac2[i]) + '\t' + str(ac2_std[i]) + '\t' + str(cc12[i]) + '\t' + str(cc12_std[i]) + '\n')
    ofile.close()
    ...
    fig = plt.figure()
    ax = plt.subplot(111)
    ax.plot(eigvec[0]*eigval[0], label = 'mode 1')
    ax.plot(eigvec[1]*eigval[1], label = 'mode 2')
    ax.plot(eigvec[2]*eigval[2], label = 'mode 3')
    ax.plot(eigvec[3]*eigval[3], label = 'mode 4')
    ax.legend(loc = 'lower left')
    plt.savefig('eigenmodes_curv.png')
    plt.close()
