...
class Performance_tools(object):
    ...
    def compute_curvature_single(self, curv, x, y, natoms):

         # transfrom arrays to ctypes
        curvc = curv.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        xc = x.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        yc = y.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        # transform integers and double to ctypes
        natomsc = ctypes.c_int(natoms)

        lib.compute_curvature_single(self.obj, curvc, xc, yc, natomsc)
        return
