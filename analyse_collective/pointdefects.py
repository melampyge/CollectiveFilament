...
class PointDefects:
    ...
    def compute_single_defect(self,xd,yd,x,y,phi,llist,head,nsegx,nsegy,lx,ly,fig_cnt):
        ...
        for a in range(-1,2):
            ...
            for b in range(-1,2):
                ...
                while val != -1:
                    ...
                    val = llist[val]
        # normalize orient function by the number of counted entries
        orient /= counter
        ...
        idxmax = np.argmax(orient)
        idxmin = np.argmin(orient)
        omax = orient[idxmax]
        omin = orient[idxmin]
        dmax = omax - omin
        dmax = 2*dmax/2/np.pi
        ...
        size_fac = 10
        plt.scatter(x,y,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)
        plt.scatter(x-lx,y-ly,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)
        plt.scatter(x,y-ly,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)
        plt.scatter(x+lx,y-ly,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)

        plt.scatter(x-lx,y,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)
        plt.scatter(x+lx,y,linewidth = 0, marker = 'o', c = color, s = size_fac, cmap = cmap)
