...
#        print i, '\t', grp
        ...
        for j, f in enumerate(grp['folder']):
            ...
            xi_l = float(grp[constants[1]].iloc[j])
            th_val = np.sqrt( 2*xi_l - 2*(xi_l**2)*(1-np.exp(-1/xi_l)))
            C_th.append(th_val)
            ...
            if os.path.exists(f):
                data = open(f, 'r')
                data.readline()  # Comment line
                data.readline()  # Empty line
                line = data.readline()
                ll = line.split()
                C.append( float(ll[-1])/body_length )
                line = data.readline()
                ll = line.split()
                C_std.append( float(ll[-1]) )
            else:
                C.append( -1 )
                C_std.append( 0 )
            ...
            if C_cnt == np.size(fp):
                ...
                c = next(color)
                print np.shape(x), '\t', np.shape(C), '\t', np.shape(C_std), '\n'
#                ax[subcnt].errorbar(x, C, yerr=C_std, marker='o', \
#                label=labels[constants[1]] + " = " + "{0:.1f}".format(grp[constants[1]].iloc[j]), color=c)
#                ax[subcnt].set_xscale('log')
#                ax[subcnt].set_yscale('log')
                ax[subcnt].loglog(x, C, '--', marker='o', \
                label=labels[constants[1]] + " = " + "{0:.1f}".format(grp[constants[1]].iloc[j]), color=c)
                ax[subcnt].loglog(x, C_th, label="_nolegend_", color=c)                
#                ax[subcnt].loglog(x, C_th,  \
#                label=labels[constants[1]] + " = " + "{0:.1f}".format(grp[constants[1]].iloc[j]) + "_", color=c)
                #ax[subcnt].plot(x, C, linewidth = 1., color=c)
                C = []
                C_std = []
                C_th = []
                x = []
        ...
        weight='bold', size=20 )
        ax[subcnt].legend(bbox_to_anchor=(1.1,1), loc=2, borderaxespad=0., prop={'size': 12})
        ax[subcnt].set_xlabel(labels[constants[0]], size=20)
        ax[subcnt].set_ylabel('$\\sqrt{<r_{e}^{2}>}/L$', size=20)
        #ax[subcnt].set_xlim((minx-5,maxx+5))
        ax[subcnt].set_ylim((0.3,1.1))
        #ax[subcnt].xaxis.set_ticks( np.arange(int(minx),int(maxx),int((maxx-minx)/4)) )
        ax[subcnt].yaxis.set_ticks( [3*10**-1, 1] )
        ax[subcnt].tick_params(axis='both', which='major', labelsize=15)
    ...
    #    'Corr. Length of Polarity', size='xx-large', weight='bold')
        ...
        path2 = args.savefile + '/plots/phase/E2E'
        ...
        plt.savefig(path2 + '/set_' + \
            labels[compareby] + "_" + "{0:.2f}".format(i) + '.png', dpi=200, bbox_inches='tight', pad_inches=0.08)
