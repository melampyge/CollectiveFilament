...
def main(): 
    ...
    loadSimData(args.datafile+'/density_0.2/kappa_5.0/fp_0.0003/init_info.txt')
...
#    fp = [0.0, 0.0048, 0.0112, 0.024, 0.08, 0.24, 0.8, 1.2, 2.4, 7.0]
    ...
    for i, grp in group:
        ...
        if i0prev != i[0]:
            ...
            path2 = args.savefile + '/plots/SPIRALS'
...
#                    y[el] = y[el]/0.05/ysum
            ...
            save_data(f, x, y, 'SPIRALS')
            
            # Plot the data to the subplot
            c = next(color)
            
            ax[subcnt].loglog(x,y, label=labels[compareby] + "{0:.1f}".format(grp['xi_L'].iloc[j]), linewidth=2., color=c)
            #print np.sum(y[:-1]*np.diff(x))
        ...
        if maxx != 0:
            ...
            #ax[subcnt].xaxis.set_ticks( [0.1, 1, 10] )
            #ax[subcnt].yaxis.set_ticks( [10**-6, 10**-3, 10] )            
            #ax[subcnt].yaxis.set_ticks( np.linspace(miny,maxy,num=5,endpoint=True, dtype=int) )
        ...
        ax[subcnt].set_xlabel('|s|', weight='bold', size=30)
        ax[subcnt].set_ylabel('P(|s|)', weight='bold', size=30)
                    
        if subcnt == i0tot:
            plt.figtext((subp.xbeg+ax_len+ax_sep)/3, subp.ybeg+ax_len+0.2, \
            'Spiral Number Distribution, ' + labels[constants[0]] + ' = ' + "{0:.2f}".format(i0prev), size=35, weight='bold')
