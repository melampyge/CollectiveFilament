...
#    save_base = '/usr/users/iff_th2/duman/RolfData/many_polymers_5'
    data_base = '/local/duman/SIMULATIONS/long_filaments/'
    save_base = '/usr/users/iff_th2/duman/RolfData/many_polymers_5'
    ...
    for d in density: 
        ...
        for k in kappa:
            ...
            for f in fp:
                ...
                path = generate_path(str(d),str(k),str(f),"SPIRALS")
        ...
        for i, k in enumerate(kappa):
            ...
            color = iter(plt.cm.rainbow(np.linspace(0,1,len(fp)+1)))
            
#            pe = 52
#            c = next(color)
#            path = generate_path(str(0.08),str(1.25),str(20.0),"SPIRALS")
#            tempdata = np.transpose(np.loadtxt(path, dtype=float))
#            tempx = tempdata[0]
#            tempy = tempdata[1] 
#            ax[i].semilogy(tempx,tempy,'o', \
#                label=labels['Pe']+'{0:.0f}'.format(pe),linewidth=3.,color=c, markersize=10) 
            ...
            for j, f in enumerate(fp):
                ...
                ax[i].semilogy(x,yf[i][j], \
                    label=labels['Pe']+'{0:.0f}'.format(pe),linewidth=3.,color=c)
#                ax[i].semilogy(x,yf[i][j], \
#                    label='_nolegend_',linewidth=2.,color=c)                    
                  
                print np.sum(yf[i][j][:-1]*np.diff(x))
 
                 
                
            pe = 10**4
            c = next(color)
            path = generate_path(str(0.08),str(1.25),str(16.0),"SPIRALS")
            tempdata = np.transpose(np.loadtxt(path, dtype=float))
            tempx = tempdata[0]
            tempy = tempdata[1] 
            ax[i].semilogy(tempx,tempy, \
                label=labels['Pe']+'{0:.0f}'.format(pe),linewidth=3.,color=c) 
            ...
            ax[i].set_xlabel('$|s|$', weight='bold', size=40)
            ax[i].set_xlim((0,8))
            ax[i].set_ylim((10**-7, 10)) 
            ax[i].xaxis.set_ticks( [0., 2., 4., 6., 8.] )
            ax[i].yaxis.set_ticks( [10**-6, 10**-4, 10**-2, 10**0])
            ax[i].tick_params(axis='both', which='major', labelsize=30)
        
            
        ax[0].set_ylabel('$P(|s|)$', weight='bold', size=40)
    
        #ax[0].legend(bbox_to_anchor=(1.01,0.,0.27,1.), loc=2, borderaxespad=0., prop={'size': 15}, mode="expand", frameon=True)
        ax[0].legend(bbox_to_anchor=(0.45,-0.6,0.27,1.), loc=2, borderaxespad=0., prop={'size': 22}, mode="expand", frameon=False)
