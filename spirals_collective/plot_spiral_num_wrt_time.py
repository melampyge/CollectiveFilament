...
def plot_data(folderbase, savebase):
    ...
    savepath = savebase + 'spiral_num_wrt_time.png'
    textpath = '/SPIRALS/spiral_tag.data'

    ## GAS PHASE
    
    data_gas_path = datapath + 'kappa_5.0/fp_0.075'
    fullpath = data_gas_path + textpath
    time, s_gas = read_data(fullpath)
    ...
    time, s_loop = read_data(fullpath)
    ...
    time, s_spiral = read_data(fullpath)
    ...
    ax0.plot(time, s_gas, label="Gas phase", linewidth=2.0)
    ax0.plot(time, s_loop, label="Loop phase", linewidth=2.0)
    ax0.plot(time, s_spiral, label="Spirals phase", linewidth=2.0)    
